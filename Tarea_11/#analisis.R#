...
ggplot(data = datos, aes(x=factor(NE), y=as.numeric(paste(T)))) + labs( x="Cantidad de soluciones", y="Tiempo" ) + geom_boxplot(aes(fill=Tipo)) + theme(text = element_text(size=30))
graphics.off()

png(paste("violinProcentaje_NE",".png", sep=""), width = 1440, height = 960, units = "px", pointsize = 10)
ggplot(data = datos, aes(x=factor(replica), y=P)) + labs( x="Cantidad de funciones objetivo", y="Porcentaje de soluciones no dominadas" ) + geom_violin(fill="orange", color="red") + geom_boxplot(width=0.1, fill="blue", color="white", lwd=1) + theme(text = element_text(size=30))
graphics.off()
