...
for(repli in 1:replicas){
    ...
    tim = proc.time()[3] - tim
    reptime = rbind(reptime, c(repli,tim, ((optimo - mejor) / optimo), (tmax-sum(gap<=min(gap)+sd(gap)) ) ) )
    print(tim)
...
colnames(reptime)= c("replica_","time","gap")
write.csv(reptime,"resultsE1wg.csv")
print(reptime)
